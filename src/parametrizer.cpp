...
#undef sE2E
    ...
    void Parametrizer::find_position_singularities() {
        ...
        m_singularity_index.resize(6, F.cols());
        for (int f = 0; f < F.cols(); ++f) {
            ...
            m_singularity_rank(0, f) = best[0];
            m_singularity_rank(1, f) = best[1];
            m_singularity_rank(2, f) = best[2];
            ...
            for (int k = 0; k < 3; ++k) {
                ...
                index += diff;
                m_singularity_index(k * 2, f) = diff[0];
                m_singularity_index(k * 2 + 1, f) = diff[1];
            }

            if (index != Vector2i::Zero()) {
                m_singularity_position[f] = rshift90(index, best[0]);
            }
        }
    }
